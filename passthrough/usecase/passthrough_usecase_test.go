...
var (
	...
	defaultBalances = sdk.NewCoins(
		sdk.NewCoin(validGammShareDenom, validGammShareAmount),
	).Add(nonShareDefaultBalances...)
...
func (s *PassthroughUseCaseTestSuite) TestGetAllBalances() {
	...
	tests := []struct {
		...
		{
			name: "skip error in parsing uint",

			address: defaultAddress,

			mockAllBalancesIfDefaultAddress: nonShareDefaultBalances.Add(sdk.NewCoin(fmt.Sprintf("%s/pool/%s", usecase.GammSharePrefix, "notuint"), validGammShareAmount)),
		...
		{
			name: "skip no forward slash in denom",

			address: defaultAddress,

			mockAllBalancesIfDefaultAddress: nonShareDefaultBalances.Add(sdk.NewCoin(usecase.GammSharePrefix, validGammShareAmount)),
