...
func NewSideCarQueryServer(appCodec codec.Codec, config domain.Config, logger log.Logger) (SideCarQueryServer, error) {
	...
	poolsUseCase := poolsUseCase.NewPoolsUsecase(timeoutContext, config.Pools, config.ChainGRPCGatewayEndpoint)
	...
	chainInfoUseCase := chaininfousecase.NewChainInfoUsecase(timeoutContext, chainInfoRepository)
	...
	if grpcIngesterConfig.Enabeld {
		...
		ingestUseCase, err := ingestusecase.NewIngestUsecase(poolsUseCase, routerUsecase, chainInfoRepository, routerRepository, tokensUseCase, appCodec, *config.Pricing, logger)
		if err != nil {
			return nil, err
		}
