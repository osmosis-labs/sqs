...
func NewSideCarQueryServer(appCodec codec.Codec, config domain.Config, logger log.Logger) (SideCarQueryServer, error) {
	...
	chainPricingStrategy, err := pricing.NewPricingStrategy(*config.Pricing, tokensUseCase, routerUsecase)
	if err != nil {
		return nil, err
	}

	// Register pricing strategy on the tokens use case.
	tokensUseCase.RegisterPricingStrategy(domain.ChainPricingSourceType, chainPricingStrategy)
