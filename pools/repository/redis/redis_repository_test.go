...
var _ = Describe("RedisRepository", func() {
	...
	var (
		...
		ctx                   context.Context
		mockCFMMPool          *mocks.CFMMPoolI
		mockConcentratedPool  *mocks.ConcentratedPoolI
		mockCosmWasmPool      *mocks.CosmWasmPoolI
	)
	...
	BeforeEach(func() {
		...
		mockCFMMPool = mocks.NewCFMMPoolI(GinkgoT())

		// configure Concentrated pool mock
		mockConcentratedPool = mocks.NewConcentratedPoolI(GinkgoT())

		// configure CosmWasm pool mock
		mockCosmWasmPool = mocks.NewCosmWasmPoolI(GinkgoT())
		...
		Context("StoreCFMM", func() {
			...
			When("storing non-CFMM pools", func() {
				It("should fail", func() {
					...
					Expect(err).To(Equal(redis.InvalidPoolTypeError{int32(types.Concentrated)}))
				})
			})
		})
		...
		Context("StoreConcentrated", func() {
			...
			When("storing non-Concentrated pools", func() {
				It("should fail", func() {
					...
					Expect(err).To(HaveOccurred())
					Expect(err).To(Equal(redis.InvalidPoolTypeError{int32(types.Balancer)}))
				})
			})
		})
