...
func NewPoolsUsecase(timeout time.Duration, redisRepositoryManager repository.TxManager, poolsConfig *domain.PoolsConfig, nodeURI string) mvc.PoolsUsecase {
...
func (p *poolsUseCase) GetAllPools(ctx context.Context) (pools []sqsdomain.PoolI, err error) {
...
func (p *poolsUseCase) GetRoutesFromCandidates(ctx context.Context, candidateRoutes sqsdomain.CandidateRoutes, takerFeeMap sqsdomain.TakerFeeMap, tokenInDenom, tokenOutDenom string) ([]route.RouteImpl, error) {
	...
	for _, candidateRoute := range candidateRoutes.Routes {
		...
		for _, candidatePool := range candidateRoute.Pools {
			pool, err := p.GetPool(ctx, candidatePool.ID)
			if err != nil {
				return nil, err
			}
