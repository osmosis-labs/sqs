...
func (s *RouterTestHelper) SetupRouterAndPoolsUsecase(router *routerusecase.Router, mainnetState MockMainnetState, rankedRoutesCache *cache.Cache, candidateRouteCache *cache.Cache) MockMainnetUsecase {
	...
	poolsUsecase := poolsusecase.NewPoolsUsecase(time.Hour, nil, &DefaultPoolsConfig, "node-uri-placeholder")
	...
	ingestUsecase, err := ingestusecase.NewIngestUsecase(&redisrepo.RedisTxManager{}, poolsUsecase, routerUsecase, nil, &routerRepositoryMock, tokensUsecase, encCfg.Marshaler, mainnetState.PricingConfig, &log.NoOpLogger{})
	if err != nil {
		panic(err)
	}
