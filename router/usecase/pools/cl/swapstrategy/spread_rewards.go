...
type spreadFactorOverOneMinusSpreadFactorGetter func() float64
...
func computeSpreadRewardChargePerSwapStepOutGivenIn(hasReachedTarget bool, amountIn, amountSpecifiedRemaining, spreadFactor float64, SfOveroneMinSf spreadFactorOverOneMinusSpreadFactorGetter) float64 {
	if spreadFactor == 0 {
		return 0
	} else if spreadFactor < 0 {
		// This should never happen but is added as a defense-in-depth measure.
		panic(fmt.Errorf("spread factor must be non-negative, was (%f), if figure is more than -1 this panic is known and not an issue", spreadFactor))
	}

	var spreadRewardChargeTotal float64
	if hasReachedTarget {
		...
		spreadRewardChargeTotal = amountSpecifiedRemaining - amountIn
	}

	if spreadRewardChargeTotal < 0 {
		// This should never happen but is added as a defense-in-depth measure.
		panic(fmt.Errorf("spread factor charge must be non-negative, was (%f), if figure is more than -1 this panic is known and not an issue", spreadRewardChargeTotal))
	}
...
func computeSpreadRewardChargeFromAmountIn(amountIn, spreadFactorOverOneMinusSpreadFactor float64) float64 {
	return math.Ceil(amountIn*spreadFactorOverOneMinusSpreadFactor*1e18) / 1e18
}
